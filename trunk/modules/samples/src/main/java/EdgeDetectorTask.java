...
public class EdgeDetectorTask implements SiyapathTask {
    ...
    public void setData(byte[] data) {
        ...
        xPos = data[0];
        yPos = data[1];
        byte[] imgData = new byte[data.length - 2];
        System.arraycopy(data, 2, imgData, 0, imgData.length);
        try {
            Toolkit.getDefaultToolkit().createImage(imgData);
            sourceImage = ImageIO.read(new ByteArrayInputStream(imgData));
        } catch (IOException e) {
            e.printStackTrace();
        }
    ...
    /*
     * This algorithm implementation is used from
